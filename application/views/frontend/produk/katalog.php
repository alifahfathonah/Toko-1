...
<div class="col-lg-12">
	...
	<div class="shop_content">
		...
		<div class="product_grid">
			...
			<!-- Product Item -->
			<?php 
			foreach($katalog_data as $katalog){ ?>
			...
			<div class="product_item discount">
				...
				<div class="product_image d-flex flex-column align-items-center justify-content-center">
					...
					else { echo " <img src='".base_url()."assets/images/produk/".$katalog->foto.'_thumb'.$katalog->foto_type."'> ";}
					?>
				</div>
				<div class="product_content">
					<div class="product_price"> <?php echo number_format($katalog->harga_diskon) ?><span>Rp <?php echo number_format($katalog->harga_normal) ?></span></div>
					<div class="product_name">
						<div><a href="<?php echo base_url("produk/$katalog->slug_produk ") ?>" tabindex="0"><?php echo character_limiter($katalog->judul_produk,20) ?></a>
						</div>
					</div>
					<div>
						<a href="
						<?php if(isset($_SESSION['identity']) && $_SESSION['usertype'] == '5'){ ?>
						<?php echo base_url('cart/buy/')?><?php echo $katalog->id_produk ?>
						<?php } else { ?>
						<?php echo base_url('auth/register') ?>
						<?php } ?>
						"><button class="btn btn-sm btn-outline-primary"><i class="fas fa-shopping-cart"></i> Beli</button></a>
						<a href="<?php echo base_url('produk/').$katalog->slug_produk ?>"><button class="btn btn-sm btn-outline-dark"><i class="fas fa-eye"></i> Detail</button></a>
					</div>
				</div>
				<ul class="product_marks">
					<li class="product_mark product_discount">-<?php echo $katalog->diskon ?>%</li>
				</ul>
			</div>
			<?php }
