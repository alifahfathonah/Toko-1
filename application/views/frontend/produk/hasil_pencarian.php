...
<div class="col-lg-12">
	<div class="shop_content">
		...
		<div class="product_grid">
			...
			<!-- Product Item -->
			<?php 
			foreach($hasil_pencarian as $value){ ?>

			<div class="product_item discount">
				<div class="product_border"></div>
				<div class="product_image d-flex flex-column align-items-center justify-content-center">
					<?php
					if(empty($value->foto)) {echo "<img class='card-img-top' src='".base_url()."assets/images/no_image_thumb.png'>";}
					else { echo " <img src='".base_url()."assets/images/produk/".$value->foto.'_thumb'.$value->foto_type."'> ";}
					?>
				</div>
				<div class="product_content">
					<div class="product_price"> <?php echo number_format($value->harga_diskon) ?><span>Rp <?php echo number_format($value->harga_normal) ?></span></div>
					<div class="product_name">
						<div><a href="<?php echo base_url("produk/$value->slug_produk ") ?>" tabindex="0"><?php echo character_limiter($value->judul_produk,20) ?></a>
						</div>
					</div>
					<div>
						<a href="
						<?php if(isset($_SESSION['identity']) && $_SESSION['usertype'] == '5'){ ?>
						<?php echo base_url('cart/buy/')?><?php echo $value->id_produk ?>
						<?php } else { ?>
						<?php echo base_url('auth/register') ?>
						<?php } ?>
						"><button class="btn btn-sm btn-outline-primary"><i class="fas fa-shopping-cart"></i> Beli</button></a>
						<a href="<?php echo base_url('produk/').$value->slug_produk ?>"><button class="btn btn-sm btn-outline-dark"><i class="fas fa-eye"></i> Detail</button></a>
					</div>
				</div>
				<ul class="product_marks">
					<li class="product_mark product_discount">-<?php echo $value->diskon ?>%</li>
				</ul>
			</div>
			<?php }
